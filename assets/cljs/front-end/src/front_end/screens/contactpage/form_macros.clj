...
(defmacro form "Defines a form and the hooks into it"
  ...
  (let [rest (map #(macroexpand %) forms)
     ...
     `(do
        ...
        #_(defn ~'do-validate-field
             "Returns an error map of all the errors generated by running the triggers on the field"
             [~'triggers ~'errors ~'store]
          (let [~'results (for [~'t ~'triggers] (~'t ~'errors ~'store))]
            (apply merge-with #(vec [%1 %2]) ~'results)
            )
          )
        #_(defn ~'refresh-validate
          "Generates a map of errors, and then merges it all back together"
          [~'values]
          (let [ ~'contents (filter #(not= :tag %) (keys ~'values))
                  ]
            (if (empty? ~'contents))
                  ~'values
                  (let [ ~'new-errors
                        (for [[~'k ~'v] ~'values]
                          (if (= :field (:tag ~'v))
                                [~'k (~'do-validate-field (:triggers ~'v))]  
                                [~'k (~'refresh-validate ~'v)] 
                                                              ))
                        ]
                    ; Now we have a map of all errors generated at this level
                    (apply merge-with #() ~'errors)
                    )
              )
          ~'values
          )
          (extend-type ~name
              ...
              (refresh [~'this]
                (let [~'values-atom (:values ~'this)
                   ...
                   nil
                   #_(reset! ~'values-atom (~'refresh-validate ~'values))
                  )
                )
              )
        ))
